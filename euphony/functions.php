...
if (! function_exists('euphony_setup')) :
	...
	function euphony_setup()
	{
		...
		add_image_size('euphony-archive-landscape', 580, 435, true); // Fixed Landscape
		add_image_size('euphony-archive-portrait', 580, 773, true); // Fixed Portrait
...
if (! function_exists('euphony_template_redirect')) :
	...
	function euphony_template_redirect()
	{
		...
		if (is_singular() && 'no-sidebar' === $layout) {
...
function euphony_scripts()
{
	...
	// Theme stylesheet.
	wp_enqueue_style('euphony-style', get_stylesheet_uri(), null, date('Ymd-Gis', filemtime(get_template_directory() . '/style.css')));
...
if (! function_exists('euphony_excerpt_more')) :
	...
	function euphony_excerpt_more($more)
	{
		...
		$link = sprintf(
			'<p class="more-link"><a href="%1$s" class="readmore">%2$s</a></p>',
...
if (! function_exists('euphony_custom_excerpt')) :
	...
	function euphony_custom_excerpt($output)
	{
		if (has_excerpt() && ! is_attachment()) {
			...
			$link = sprintf(
				'<a href="%1$s" class="more-link"><span class="readmore">%2$s</span></a>',
				esc_url(get_permalink()),
				/* translators: %s: Name of current post */
				wp_kses_data($more_tag_text) . '<span class="screen-reader-text">' . get_the_title(get_the_ID()) . '</span>'
			);
